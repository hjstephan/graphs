...
\begin{itemize}
	\item[-] \textbf{Primäre Richtung}: Rechtsherum (clockwise) $\equiv$ negative Winkelrichtung
	\item[-] \textbf{Sekundäre Richtung}: Feedback-Verbindungen (counter-clockwise)
...
\begin{figure}[h]
	...
	\caption{Richtungsvergleich: Rechtsherum (Forward) vs. Linksherum (Backward). (links) Forward- und Backward-Pfadlängen. Feed-forward Netzwerke haben kürzere Forward-Pfade. (rechts) Direktionalitäts-Ratio. Werte > 1.0 bedeuten Forward-Dominanz, Werte ≈ 1.0 bedeuten Symmetrie.}
